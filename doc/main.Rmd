...
for (k in 1:length(file_name_vec)){
  ...
  for (k in 1:length(file_name_vec)){
    ...
    total_detect_output[[k]] <- current_d1_output[[1]]
...
```{r}
set.seed(2018)
selected_index <- sample(1:100,5)
current_ground_truth_txt <- list(c(total_ground_truth_txt_100[[selected_index[1]]],
                                   total_ground_truth_txt_100[[selected_index[2]]],
                                   total_ground_truth_txt_100[[selected_index[3]]],
                                   total_ground_truth_txt_100[[selected_index[4]]],
                                   total_ground_truth_txt_100[[selected_index[5]]]))

current_tesseract_txt <- list(c(total_tesseract_txt_100[[selected_index[1]]],
                                total_tesseract_txt_100[[selected_index[2]]],
                                total_tesseract_txt_100[[selected_index[3]]],
                                total_tesseract_txt_100[[selected_index[4]]],
                                total_tesseract_txt_100[[selected_index[5]]]))

current_tesseract_clean_txt <- list(c(total_tesseract_clean_output[[selected_index[1]]],
                                total_tesseract_clean_output[[selected_index[2]]],
                                total_tesseract_clean_output[[selected_index[3]]],
                                total_tesseract_clean_output[[selected_index[4]]],
                                total_tesseract_clean_output[[selected_index[5]]]))
...
corpus_matrix <- matrix(corpus,nrow = length(corpus),ncol = 1)
...
for(i in error_words){
  ...
  total_candidate[[word]] <- unique(corpus_matrix[index,])
}
)
...
comparison <- rbind(total_detect_output[[selected_index[1]]],
                    total_detect_output[[selected_index[2]]],
                    total_detect_output[[selected_index[3]]],
                    total_detect_output[[selected_index[4]]],
                    total_detect_output[[selected_index[5]]]
                    )
...
Basic evaluation for quick analysis
```{r, warning=FALSE, message = FALSE}
evaluate <- c()
for(i in names(final_result)){
  if(filter(comparison,tesseract_err == i)[,2] %in% final_result[[i]]>0){
    evaluate <- c(evaluate,1)
  }
  else{evaluate <- c(evaluate,0)}
}
mean(evaluate)
```
